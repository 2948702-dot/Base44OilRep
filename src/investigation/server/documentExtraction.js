...
export async function extractDocument(buffer, meta = {}) {
  ...
  if (mime.startsWith('image/')) {
    // Изображение не разбирается здесь: сначала распознавание текста (задача ocr),
    // и только потом разбор распознанного текста. Обратный порядок разбирал бы
    // байты картинки как символы.
    throw new Error(
      'Изображение разбирается через распознавание текста: поставьте задачу ocr. '
      + 'Оригинал сохранён и не изменяется.',
    );
  }
