...
/**
 * Минимальный сборщик ZIP: только то, что нужно для OOXML.
 *
...
function buildZip(entries) {
  ...
  for (const entry of entries) {
    ...
    // Отметка времени фиксированная: выгрузка одного и того же отчёта обязана давать
    // одинаковый файл, а нулевая дата (месяц 0, день 0) недопустима в ZIP, и часть
    // читателей отказывается открывать такой архив.
