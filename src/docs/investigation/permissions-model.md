...
- Журналы (аудит, запуски агентов, история версий, результаты бенчмарка) неизменяемы
  всегда. Единственное исключение — удаление арендатора, и оно требует одновременно
  флага `app.tenant_erasure` и прав владельца таблиц. Роль приложения владельцем не
  является: выставить флаг она может, стереть журнал — нет.
- Факт удаления арендатора остаётся в `tenant_deletion_record`: организация, дата,
  основание, заказчик, число удалённых строк и контрольная сумма выданной выгрузки.
  Персональных данных запись не содержит и доступна только системному администратору.
