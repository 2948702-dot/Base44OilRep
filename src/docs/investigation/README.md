...
## Данные арендатора (§60 ТЗ)

Организация вправе забрать свои материалы и вправе потребовать их удаления.
Обе процедуры выполняются на сервере и не имеют HTTP-маршрута: у них нет отмены,
и они должны требовать доступа к серверу, а не одной кнопки в браузере.
...
ADMIN_DATABASE_URL=... node investigation/tools/tenant-data.mjs --delete \
  --slug <slug> --confirm <slug> --reason "..." --requested-by "..."
```

Выгрузка идёт под областью видимости самой организации: row-level security не
отключается, поэтому архив физически не может захватить чужие данные. Архив
самоописателен — манифест, данные по таблицам, оригиналы материалов по хэшу
и пояснение к структуре.

Удаление требует подключения владельца схемы: журналы стирает только он, и роль
приложения не сотрёт их, даже выставив флаг стирания. Факт удаления записывается
в `tenant_deletion_record` и переживает сами данные.

Через GitHub Actions обе процедуры доступны в «DATA - export or delete investigation
tenant». Архив остаётся на сервере и в артефакты сборки не попадает.
