...
| DOCX | абзац |
| Сканы и фотографии (PNG, JPEG, TIFF) | строка распознанного текста; распознавание идёт отдельным шагом до разбора |
| XLSX и устаревший Office | не разбираются: отказ явный, оригинал сохранён |

Нераспознанный двоичный файл отклоняется, а не декодируется как текст: молчаливый разбор
мусора даёт утверждения, которых в документе нет, и следователь видит «разбор выполнен».
...
Изображение сначала распознаётся (задача `ocr`), и только потом разбирается как текст.
Обратный порядок разбирал бы байты картинки как символы. Распознанный текст живёт
производным источником рядом с изображением и несёт пометку о том, что читал его
не человек, и с какой уверенностью: вывод, опирающийся на распознанный текст низкого
качества, должен быть виден как таковой.

## 19. Case Director — агент симулятора

Работает только в симуляторе учебных дел и в производственном расследовании не
участвует. Единственный агент, которому известна скрытая истина учебного дела, и
единственный, кому разрешено читать `TrainingCase`.

Играет участников и держателей материалов: отвечает на вопросы интервью и решает, выдать
ли запрошенный документ. Не помогает расследованию и не мешает ему — воспроизводит мир,
в котором расследование ведётся.

Ограничения не оставлены на добросовестность промпта:

| Ограничение | Как обеспечено |
|---|---|
| не сообщает, кто виноват | в схеме ответа нет поля, куда это можно записать |
| не выдумывает материал | выбирает элемент `evidence_sequence` по идентификатору; содержимое подставляется из учебного дела |
| не раскрывает скрытую истину в реплике | список `never_reveal_in_dialogue` проверяется кодом после каждого ответа; нарушение останавливает прогон |

Подробнее — `simulator.md`.
