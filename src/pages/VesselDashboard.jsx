...
export default function VesselDashboard() {
  ...
  const { data: thresholdRules = [] } = useQuery({ queryKey: ['threshold-rules'], queryFn: () => base44.entities.ThresholdRule.list() });
                        ...
                        return (
                          <div key={g.key} className="bg-slate-50 rounded-xl border border-slate-100 px-2 py-2">
                            <ParameterGauge
                              ...
                              min={ruleZones?.min ?? g.min}
                              max={ruleZones?.max ?? g.max}
                              zones={ruleZones?.zones ?? g.zones}
                              decimals={g.decimals}
                            />
                          </div>
                        );
