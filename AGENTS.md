...
1. Проверить текущую ветку, `git status` и последние коммиты.
2. Прочитать `ROADMAP.md` и `src/docs/known-issues.md`.
3. При необходимости сверить текущий P0 с `decisions.md`, бизнес-правилами, реальным кодом и production read-only данными.
4. Если пользователь не задал другую конкретную задачу или приоритет, начать с первого незакрытого пункта P0.
5. Не считать запись в документации автоматически истинной, если код или production ей противоречат: зафиксировать расхождение и работать от подтверждённого состояния.
6. После завершения обновить статус, решения, известные проблемы и журнал проверок в соответствующих документах.
